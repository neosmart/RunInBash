...
std::vector<std::wstring> Parse(const TCHAR *cmdLine)
{
	...
	if (CreatePipe(&bash_stdout[PIPE_READ], &bash_stdout[PIPE_WRITE], nullptr, 0) == 0)
	{
	...
	if (CreatePipe(&bash_stderr[PIPE_READ], &bash_stderr[PIPE_WRITE], nullptr, 0) == 0)
	{
		_ftprintf(stderr, _T("CreatePipe: 0x%x\n"), GetLastError());
		ExitProcess(-1);
	}
	...
	HANDLE bash_stdin[2]{};
	if (CreatePipe(&bash_stdin[PIPE_READ], &bash_stdin[PIPE_WRITE], nullptr, 0) == 0)
	{
		_ftprintf(stderr, _T("CreatePipe: 0x%x\n"), GetLastError());
		ExitProcess(-1);
	}
	...
	{
		_ftprintf(stderr, _T("SetHandleInformation(bash_stdin[1]): 0x%x"), GetLastError());
		ExitProcess(-1);
	}
	...
	auto script = sprintf(_T("for arg in $(echo \"%s\"); do echo hello\\n ; done"), args);
	_tprintf(_T("script: %s\n"), script.c_str());
	CreateProcess(bash, (LPWSTR)(sprintf(_T("bash -c \"%s\""), script.c_str()).c_str()), nullptr, nullptr, true, 0, nullptr, currentDir, &startInfo, &pInfo);

	//Close the write handle to stdin
	CloseHandle(bash_stdin[PIPE_WRITE]);

	std::wstring stdout_str;
	uint32_t stdout_addr;
	HANDLE hThread = reinterpret_cast<HANDLE>(CreateThread(0, 0,
		static_cast<LPTHREAD_START_ROUTINE>([bash_stderr](void* pData) -> unsigned int {
		std::wstringstream output;
		while (true)
		{
			DWORD bytesRead;
			TCHAR buffer[512];
			if (!ReadFile(bash_stderr[PIPE_READ], buffer, _countof(buffer) * sizeof(TCHAR), &bytesRead, nullptr))
			{
				_ftprintf(stderr, _T("Error reading from bash stdout: 0x%x\n"), GetLastError());
				ExitProcess(-1);
			}
	...
	}), nullptr, 0, &stdout_addr));



	auto arguments = StringSplit(output.str(), _T("\n"), true);
}

bool contains_any_of(const TCHAR *haystack, const TCHAR *charArray)
{
	int searchLength = _tcsclen(charArray);
...
int _tmain(int argc, TCHAR *argv[])
{
	...
	for (auto arg : args)
	{
		...
		if (debugLevel > 1)
		{
			_ftprintf(stderr, _T("mapped argument: %s\n"), translatedPath);
		}
