...
template <typename T> const TCHAR *GetArgumentString(const T argv) {
...
std::vector<std::wstring> inline StringSplit(const std::wstring &source,
...
TCHAR *TranslatePath(const TCHAR *winPath, bool &fileFound) {
    ...
    auto hFile = CreateFile(
        winPath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr,
    ...
    // > cchFilePath [in]: The size of lpszFilePath, in TCHARs. This value does
    // not include a NULL termination character. > [..] If the function fails
    ...
    // But this behavior as documented is wrong, the returned value in case of not
    // enough memory does not include the null character.
    ...
    if (buffer[0] >= _T('A') && buffer[0] <= _T('z') &&
        buffer[1] == _T(':')) // this is safe since we assert that length is at
                              // least 1 + \0 above
    {
        ...
        _stprintf_s(result, newCount, _T("/mnt/%c%s"),
                    iswupper(buffer[0]) ? towlower(buffer[0]) : buffer[0], buffer + 2);
        return result;
    }
...
int _tmain(int argc, TCHAR *argv[]) {
    ...
    bool success = CreateProcess(bash, lpArg, nullptr, nullptr, true, 0, nullptr, currentDir,
                                 &startInfo, &pInfo);
